...
\begin{multicols*}{4}
	...
	\subsection{Layered Network Design Issues}
	\begin{itemize}
	...
	\textbf{Vertical:} services for uni directional communication provided by bottom layer to top layer, like a gateway \\
	...
	\textbf{Packet Transmission Delay:} $\frac{L}{R}$ (Length in bits/Transmission rate in bits/s) \\
	...
	\textbf{SYN-ACK Process:}
	\begin{itemize}
		\item \textbf{Step 1 (SYN):} Client sends SYN packet with initial sequence number x
		      \begin{itemize}
			      \item SYN flag = 1
			      \item Sequence number = x (random)
		      \end{itemize}
		\item \textbf{Step 2 (SYN-ACK):} Server responds with SYN-ACK packet
		      \begin{itemize}
			      \item SYN flag = 1
			      \item ACK flag = 1
			      \item Sequence number = y (random)
			      \item Acknowledgment number = x+1
		      \end{itemize}
		\item \textbf{Step 3 (ACK):} Client completes handshake with ACK
		      \begin{itemize}
			      \item ACK flag = 1
			      \item Sequence number = x+1
			      \item Acknowledgment number = y+1
		      \end{itemize}
	\end{itemize}
	...
	\subsection{Routing}
	Network-wide path determination (seconds). Determine a route to move datagrams from source to destinatino. \textbf{Control plane:} network wide logic. the plane that plans over all route an ip datagram takes.\\
	...
	\includegraphics[width=\linewidth]{SCR-20250417-kkjm.png}
	...
	%--------------------------------------------------------------------
	% Packet Journeys: DHCP, DNS, and HTTP
	%--------------------------------------------------------------------
	...
	% \includegraphics[width=\textwidth, height=\textheight, keepaspectratio]{middle-finger.png}
