...
\begin{multicols*}{4}
	...
	\textbf{Gateway:} Device transferring info between layers \\
	...
	\textbf{Vertical:} Between adjacent layers (service) \\
	\textbf{Horizontal:} Between same layers (protocol)

	\subsection{Connection Types}
	\textbf{Connection-Oriented:} Setup required for usage \\
	\textbf{Connectionless:} Each message handled separately \\
	Both can be reliable or unreliable

	\subsection{OSI Model}
	\textbf{7. Application:} Network apps, end-user access (FTP, SMTP, HTTP) \\
	\textbf{6. Presentation:} Data interpretation/formatting \\
	\textbf{5. Session:} Maintains connections \\
	...
	\textbf{2. Data Link:} Node-to-node transfer, physical addressing \\
	\textbf{1. Physical:} Raw bit transmission (cables, signals)

	\subsection{TCP/IP Model}
	\textbf{Application:} TELNET, FTP, SMTP, DNS, HTTP, RTP \\
	\textbf{Transport:} TCP (reliable, connection-oriented), UDP (unreliable, connectionless) \\
	\textbf{Internet:} IP packet delivery \\
	\textbf{Link:} Hardware interface
