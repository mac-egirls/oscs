...
\begin{multicols*}{3}
    ...
    Signature of an algebra is the name of carrier and the types of its operators.
    ...
    Def. Subset $T$ of a set $S$ is closed under an operator
    if applying operator to elements of T always produces a result in T.
    Def. $\langle T, \Phi \rangle$ is a subalgebra of $\langle S, \Phi \rangle$ if $T$ is closed under
    every operator in $\Phi$.
    Thrm. A subalgebra of a group iff the inverse of every element of the subalgebra is in the subalgebra.
    ...
    A function $h$ is an isomorphism if it is one-to-one and onto,
    ...
    $A$ and $\hat{A}$ are isomorphic and $\hat{A}$ is called the isomorphic image of $A$.
    ...
    \textbf{Group Theory}
    Semigroup is an algebra $\langle S, \circ \rangle$, where $\circ$ is a binary associative operator,
    and with no identity.
    Monoid $\langle S, \circ, 1 \rangle$ is a semigroup with an identity $1$.
    Albelian monoid is if $\circ$ is symmetric.
    A subalgebra of a monoid that contains the identity of the monoid is a submonoid (which is also a monoid).
    Any semigroup can be made into a monoid by adding an identity element.
    A group is a monoid with an inverse for every element $(b^{-1})$.
    ...
    \section{Prove Axiom 9.10 and 9.11}
    \subsection{Prove Axiom 9.10}
