...
\end{lstlisting}
    ...
    % TODO: #15 Rewrite paging info as not a question
    \subsubsection{Number of Bits and/or Entries}
    \textbf{Important}: if any size is not in bytes convert to bytes first.\\
    If address is in x bits, page size is $2^y$ bytes, number of frames/pages is $2^z$, then $x = y + z$.\\
    % I think this is true? If not delete.
    conventional single level $\equiv$ virtual memory\\
    inverted $\equiv$ physical memory.
    ...
    \subsection{Directory Structure}
        ...
        directory. Is a tree of height 2. \\
