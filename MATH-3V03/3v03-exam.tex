...
\begin{multicols*}{4}
	...
	\includegraphics[width=0.45\linewidth]{petersen-graph.png}
	...
	$D_n = n!\left(\frac 1 {2!} - \frac 1 {3!} + \cdots + \frac{(-1)^{n-1}}{(n-1)!} + \frac{(-1)^n}{n!}\right)$
	...
	% \includegraphics[width=0.45\linewidth]{figure-5.3.2.png} \includegraphics[width=0.5\linewidth]{figure-5.3.3.png} \\
	...
	\textbf{Convex}: A region in a plane drawing of a planar graph s.t. whenever any two points on the boundary of or in the region are connected by a strait line segment, the straight line segment lies entirely inside the region. \\
	...
	For a complete graph $K_{n}$, the following inequality holds:
	\[ cr(K_{n}) \le \frac{1}{4}\left\lfloor\frac{n}{2}\right\rfloor\left\lfloor\frac{n-1}{2}\right\rfloor\left\lfloor\frac{n-2}{2}\right\rfloor\left\lfloor\frac{n-3}{2}\right\rfloor \]
	For a complete bipartite graph $K_{m,n}$, we have so:
	\begin{align*}
		cr(K_{2t,2s})     & \le t(t-1)s(s-1) \\
		cr(K_{2t+1,2s+1}) & \le t^{2}s^{2}   \\
		cr(K_{2t+1,2s})   & \le t^{2}s(s-1)
	\end{align*}
	\textbf{Theorem 9.1.5.} The crossing number of $K_{m,n}$ satisfies the inequality
	\[ cr(K_{m,n}) \le \left\lfloor\frac{m}{2}\right\rfloor\left\lfloor\frac{m-1}{2}\right\rfloor\left\lfloor\frac{n}{2}\right\rfloor\left\lfloor\frac{n-1}{2}\right\rfloor \]
	\textbf{Theorem 9.1.6.} The crossing number of $K_{3,n}$ is
	\[ cr(K_{3,n}) = \left\lfloor\frac{n}{2}\right\rfloor\left\lfloor\frac{n-1}{2}\right\rfloor \]
	\textbf{Theorem 9.1.7.} The crossing number of $K_{4,n}$ is
	\[ cr(K_{4,n}) = \left\lfloor\frac{n}{2}\right\rfloor\left\lfloor\frac{n-1}{2}\right\rfloor \]
	\section*{Thickness and Splitting Number}
	\textbf{Definition:} Thickness - denoted $\theta(G)$, is min number of planar subgraphs in a decomposition of $G$ into planar subgraphs. So $\theta(G)=t$ means there are decompositions of $G$ s.t.
	\[ G=H_{1}\cup \dots \cup H_{t} \]
	Where $H_{i}$ is planar, and fewer decompositions is not possible.
	\textbf{Theorem 9.2.1.} If $G$ is a graph with $p$ vertices and $q$ edges, then
	\[ \theta(G) \ge \frac{q}{3p-6} \]
	\textbf{Theorem 9.2.2.} If $G$ is a bipartite graph with $p$ vertices and $q$ edges, then
	\[ \theta(G) \ge \frac{q}{2p-4} \]
	...
	\includegraphics[width=0.45\linewidth]{figure-10.1.2.png}
	\includegraphics[width=0.45\linewidth]{figure-10.1.3.png}
	...
	Proof outline: Case that length $\geq 5$: In a circuit of length at least 5, at least one pair of vertices is not adjacent, otherwise the graph contains a subgraph isomorphic to $K_5$, which is not planar. create an edge between these two vertices, thereby creating two circuits from one and adding an edge, so this new graph is therefore planar by the planar rotation definition, meaning $G$ was not maximal.\\
	Case that length $= 4$: In a circuit of length 4, if any two vertices are not adjacent, do the same as previous case. Otherwise, create a new vertex and connect it to all 4 vertices in the cycle, thereby creating 4 circuits from one and adding 4 edges and 1 vertex, so this new graph is therefore planar by the planar rotation definition, meaning $G$ was not maximal.\\
	...
	\textbf{Mapping onto a Rectangle}: Proof of $\gamma(K_5)=\gamma(K_{3,3})=1$ (torus)
	\includegraphics[width=\linewidth]{figure-10.3.4.png} \\
	Proof of $\gamma(K_7)=\gamma(K_{4,4})=1$ (torus)
	\includegraphics[width=0.45\linewidth]{figure-10.3.6.png}
	\includegraphics[width=0.45\linewidth]{figure-10.3.7.png} \\
	...
	\textbf{Theorem 10.3.3--10.3.4 (Ringel)}: $\displaystyle \gamma(K_{m,n})=\left\lceil\frac{(m-2)(n-2)}{4}\right\rceil$ for $m,n\ge 2$. (Proof idea: bipartite $\Rightarrow$ shortest induced circuit $\ge 4$, so $2q\ge 4r(\rho)$; combine with $p-q+r(\rho)=2-2g$.) \\
	...
	\textbf{Theorem 10.3.6 (Ringel, Youngs)}: $\displaystyle \gamma(K_n)=\left\lceil\frac{(n-3)(n-4)}{12}\right\rceil$ for $n\ge 3$. \\
	\textbf{Theorem 10.3.7 (Heawood)}: If $G$ is critical with chromatic number $\chi$ and $\gamma(G)\le g$ with $g\ge 1$, then $\displaystyle \chi(G) \le \left\lfloor\frac{7+\sqrt{48g+1}}{2}\right\rfloor$ (the Heawood number $H(g)$). For $g=0$ this is not Heawood; use the Four Color Theorem instead. \\
	\textbf{Theorem 10.3.8 (Map Color Theorem)}: For every $g\ge 1$, there exists a critical graph $G$ and $\gamma(G) \ge g$ and $\displaystyle \chi (G) = \left \lfloor \frac{7 + \sqrt{1 + 48g}} 2\right \rfloor$
